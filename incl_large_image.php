...
?>
	...
	<div class="page page-quote page-<?php echo $counter; ?>"  data-src="<?php echo $src[0]; ?>" data-src-1024="<?php echo $src_1024[0]; ?>" data-src-768="<?php echo $src_768[0]; ?>" data-src-480="<?php echo $src_480[0]; ?>">
		<?php if (!empty($quote)){ ?>
			<h1 class="vert-center  <?php echo  $color_scheme;  ?>"><?php echo '&ldquo;';echo $quote;echo '&rdquo;';?></h1>
		<?php } ?>
		<?php
			if(strlen($extra_image['url'])> 1){
				//echo '<div class="extra-img vert-'.$position_vertical_extra.'-title hor-'.$position_horizontal_extra.'-title"><img src="'.$extra_image['url'].'" class=" lrg-img-over "  nopin="nopin"></div>';
